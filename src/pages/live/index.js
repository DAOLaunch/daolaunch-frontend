...
import ConnectModal from '@/layouts/navbar/connect-modal'
...
class Live extends Component {
  ...
  componentDidMount() {
    ...
    if (!!projectId) {
      if (
        ...
        && (!Storage.has('IS_DISCONNECT') || !Storage.get('IS_DISCONNECT'))
      ) {
  ...
  _getAllProject = () => {
    ...
    getAllProject({
      time: SYSTEM.TOKEN_SALES_TIMES.LIVE,
  ...
  render() {
    ...
    return (
      <>
        ...
        <ConnectModal innerRef={(ref) => { this._connectModal = ref }} />
      </>
    )
  }
}
